...
async function alertasDisciplinaYWellness(clubId: string, jugadoresMap: Map<any, any>) {
  ...
  const { data: rMapPar } = await supabase.from("partidos").select("id, categoria, fecha").eq("club_id", clubId);
  const catDePartido: Record<string, string> = {};
  (rMapPar || []).forEach((p: any) => { catDePartido[p.id] = p.categoria || "Sin categoría"; });
  const partidosTemporada = new Set(
    (rMapPar || []).filter((p: any) => {
  ...
  (tarjetas || []).forEach((t: any) => {
    ...
    if (!partidosTemporada.has(t.id_partido)) return;
    const cat = catDePartido[t.id_partido] || "Sin categoría";
    const key = `${t.id_jugador}|${cat}`;
    amarillasCat[key] = (amarillasCat[key] || 0) + 1;
  });

  const { data: sanc } = await supabase.from("disciplina_sanciones").select("*").eq("club_id", clubId);
  ...
  const { data: wHoy } = await supabase.from("wellness").select("*").eq("club_id", clubId).eq("fecha", hoyStr);
