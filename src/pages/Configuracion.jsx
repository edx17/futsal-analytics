...
function Configuracion() {
  ...
  // FORMULARIO FULL CHAMPIONS LEAGUE (AHORA CON APELLIDO)
  ...
  const handleGuardarClub = () => {
    localStorage.setItem('mi_club', clubName);
    alert('¡Nombre del club actualizado en tu dispositivo!');
  };
  ...
  const handleGuardarJugador = async () => {
    ...
    if (formData.id) {
      // Editar
      const { error } = await supabase.from('jugadores').update(formData).eq('id', formData.id);
      if (!error) {
      ...
      // Crear
      const { error } = await supabase.from('jugadores').insert([formData]);
      if (!error) {
  ...
  const jugadoresOrdenados = [...jugadores].sort((a, b) => {
    ...
    // Si ordenamos por nombre, combinamos apellido y nombre para ordenar bien
  ...
  return (
    <div style={{ animation: 'fadeIn 0.3s' }}>
      ...
      <div className="bento-card" style={{ marginBottom: '30px' }}>
        <div className="stat-label" style={{ marginBottom: '20px' }}>CONFIGURACIÓN DEL DISPOSITIVO</div>
        <div style={{ display: 'flex', gap: '15px', alignItems: 'flex-end' }}>
          <div style={{ flex: 1 }}>
            <div className="section-title">NOMBRE DE MI CLUB</div>
            <input 
              type="text" 
              value={clubName} 
              onChange={(e) => setClubName(e.target.value)} 
              style={{ ...inputIndustrial, fontSize: '1.2rem', fontWeight: 800, color: 'var(--accent)' }}
            />
          </div>
          <button onClick={handleGuardarClub} className="btn-action">GUARDAR</button>
        </div>
        <p style={{ fontSize: '0.8rem', color: 'var(--text-dim)', marginTop: '10px' }}>*Este nombre se usará por defecto en los gráficos y toma de datos.</p>
      </div>

      {/* 2. SECCIÓN: GESTIÓN DE PLANTEL */}
      ...
      {/* MODAL: VER FICHA COMPLETA DEL JUGADOR */}
      {jugadorSeleccionado && (
          ...
          <div className="bento-card modal-content" style={{ maxWidth: '600px', background: '#111' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '1px solid #333', paddingBottom: '20px', marginBottom: '20px' }}>
              <div style={{ display: 'flex', gap: '20px', alignItems: 'center' }}>
                <div style={{ fontSize: '3.5rem', fontWeight: 900, color: 'var(--accent)', fontFamily: 'JetBrains Mono', lineHeight: 1 }}>{jugadorSeleccionado.dorsal}</div>
                <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div style={{ background: '#111', padding: '15px', borderRadius: '4px', border: '1px solid #333' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '15px', marginBottom: '15px' }}>
                  <div>
                    <div className="section-title">NACIMIENTO</div>
                    <input type="date" value={formData.fechanac} onChange={e => setFormData({...formData, fechanac: e.target.value})} style={inputIndustrial} />
                  </div>
                  <div>
                    <div className="section-title">DNI</div>
                    <input type="number" value={formData.dni} onChange={e => setFormData({...formData, dni: e.target.value})} style={inputIndustrial} placeholder="Sin puntos" />
                  </div>
                  <div>
                    <div className="section-title">PESO (KG)</div>
                    <input type="number" step="0.1" value={formData.peso} onChange={e => setFormData({...formData, peso: e.target.value})} style={inputIndustrial} placeholder="Ej: 75.5" />
                  </div>
                  <div>
                    <div className="section-title">ALTURA (CM)</div>
                    <input type="number" value={formData.altura} onChange={e => setFormData({...formData, altura: e.target.value})} style={inputIndustrial} placeholder="Ej: 178" />
                  </div>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '15px', marginBottom: '15px' }}>
                  <div>
                    <div className="section-title">GRUPO SANGUÍNEO</div>
                    <input type="text" value={formData.grupo_sanguineo} onChange={e => setFormData({...formData, grupo_sanguineo: e.target.value})} style={inputIndustrial} placeholder="Ej: O+" />
                  </div>
                  <div>
                    <div className="section-title">OBRA SOCIAL</div>
                    <input type="text" value={formData.obra_social} onChange={e => setFormData({...formData, obra_social: e.target.value})} style={inputIndustrial} placeholder="Ej: OSDE" />
                  </div>
                  <div>
                    <div className="section-title">VENCIMIENTO APTO FÍSICO</div>
                    <input type="date" value={formData.vencimiento_apto} onChange={e => setFormData({...formData, vencimiento_apto: e.target.value})} style={{...inputIndustrial, borderColor: 'var(--accent)'}} />
                  </div>
                </div>
              </div>

              {/* BLOQUE CONTACTO E INDUMENTARIA */}
              <div style={{ background: '#111', padding: '15px', borderRadius: '4px', border: '1px solid #333' }}>
                <div className="section-title" style={{ marginTop: 0 }}>CONTACTO E INDUMENTARIA</div>
                
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom: '15px' }}>
                  <div>
                    <div className="section-title">TELÉFONO PERSONAL</div>
                    <input type="number" value={formData.contacto} onChange={e => setFormData({...formData, contacto: e.target.value})} style={inputIndustrial} placeholder="Ej: 1123456789" />
                  </div>
                  <div>
                    <div className="section-title">TELÉFONO EMERGENCIA</div>
                    <input type="text" value={formData.contacto_emergencia} onChange={e => setFormData({...formData, contacto_emergencia: e.target.value})} style={{...inputIndustrial, borderColor: '#ef4444'}} placeholder="Nombre y Número" />
                  </div>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                  <div>
                    <div className="section-title">TALLE DE ROPA</div>
                    <input type="text" value={formData.talla_ropa} onChange={e => setFormData({...formData, talla_ropa: e.target.value})} style={inputIndustrial} placeholder="Ej: M / L / XL" />
                  </div>
                  <div>
                    <div className="section-title">NRO. CALZADO</div>
                    <input type="number" value={formData.talla_calzado} onChange={e => setFormData({...formData, talla_calzado: e.target.value})} style={inputIndustrial} placeholder="Ej: 42" />
                  </div>
                </div>
              </div>
      ...
      {/* STYLES AUXILIARES */}
      <style>{`
        .inputIndustrial { background: transparent; border: 1px solid var(--border); }
        .modal-overlay {
...
const inputIndustrial = {
  width: '100%', padding: '12px', background: '#000', 
  border: '1px solid #333', color: '#fff', 
  borderRadius: '4px', outline: 'none', fontFamily: 'JetBrains Mono'
};

const btnGhost = {
  background: 'transparent', border: '1px solid #333', color: '#fff', 
  padding: '6px 12px', cursor: 'pointer', borderRadius: '4px', fontSize: '0.75rem', fontWeight: 800
};

const fichaRow = {
  display: 'flex', justifyContent: 'space-between', padding: '10px 0', 
  borderBottom: '1px dashed #333', color: 'var(--text-dim)', fontSize: '0.9rem'
};
