...
export default function Inicio() {
  ...
  if (isKioscoMode) {
    ...
    return (
      <div style={{ animation: 'fadeIn 0.3s', padding: esMovil ? '15px' : '20px', maxWidth: '1100px', margin: '0 auto', minHeight: '100vh' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', gap: '15px', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
            <div style={{ width: esMovil ? '45px' : '60px', height: esMovil ? '45px' : '60px', borderRadius: '50%', background: '#222', border: '2px solid var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--accent)', fontWeight: 800, fontSize: esMovil ? '1rem' : '1.5rem', overflow: 'hidden', flexShrink: 0 }}>
              {escudoK ? <img src={escudoK} alt="Escudo" style={{ width: '100%', height: '100%', objectFit: 'contain' }} /> : nombreClubK.substring(0, 2).toUpperCase()}
            </div>
            <div>
              <div className="stat-label" style={{ color: 'var(--text-dim)', fontSize: '0.7rem' }}>INGRESO JUGADOR</div>
              <h1 style={{ margin: 0, fontSize: esMovil ? '1.4rem' : '1.8rem', fontWeight: 900, textTransform: 'uppercase' }}>{nombreClubK}</h1>
            </div>
          </div>
          ...
          <button
            ...
            style={{ background: 'transparent', border: '1px solid var(--accent)', color: 'var(--accent)', padding: esMovil ? '10px 15px' : '16px 24px', fontSize: esMovil ? '0.8rem' : '0.9rem' }}
          >
            SALIR
          </button>
        </div>
        ...
        {kioscoCargando ? (
          ...
          <>
            <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '25px', flexWrap: 'wrap' }}>
              <div style={{ width: esMovil ? '55px' : '72px', height: esMovil ? '55px' : '72px', borderRadius: '50%', background: '#222', border: '2px solid var(--accent)', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, color: 'var(--accent)', flexShrink: 0 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: esMovil ? '1.5rem' : '2rem', fontWeight: 900, textTransform: 'uppercase', color: '#fff', lineHeight: 1 }}>{nombreJugador}</div>
              </div>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: esMovil ? 'repeat(2, 1fr)' : 'repeat(auto-fit, minmax(180px, 1fr))', gap: '12px' }}>
              {menuJugador.map(item => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: '16px',
                    padding: esMovil ? '15px 12px' : '22px 18px',
                    textAlign: 'left',
                    cursor: 'pointer',
                    color: '#fff',
                    minHeight: esMovil ? '100px' : '120px',
                    display: 'flex',
                    flexDirection: 'column',
                    justifyContent: 'center'
                  }}
                >
                  <div style={{ fontSize: esMovil ? '1.8rem' : '1.6rem', marginBottom: '8px' }}>{item.icon}</div>
                  <div style={{ fontSize: esMovil ? '0.75rem' : '0.82rem', letterSpacing: '0.5px', color: 'var(--text-dim)', fontWeight: 800, marginBottom: '4px' }}>{item.titulo}</div>
                </button>
              ))}
            </div>
          </>
  ...
  return (
    <div style={{ animation: 'fadeIn 0.3s', maxWidth: '1100px', margin: '0 auto', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', flexDirection: esMovil ? 'column' : 'row', justifyContent: 'space-between', alignItems: esMovil ? 'flex-start' : 'center', marginBottom: '25px', paddingBottom: '20px', borderBottom: '1px solid var(--border)', gap: '15px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px', width: '100%' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <h1 style={{ margin: 0, fontSize: esMovil ? '1.4rem' : '1.8rem', fontWeight: 900, textTransform: 'uppercase', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{nombreClub}</h1>
          </div>
        </div>
        
        {/* Parte Inferior (Filtros y Edición) - En móvil toma el ancho completo */}
        <div style={{ display: 'flex', flexDirection: esMovil ? 'column' : 'row', alignItems: esMovil ? 'stretch' : 'center', gap: '10px', width: esMovil ? '100%' : 'auto' }}>
            
            {/* SELECTOR DE CATEGORÍA */}
            {rol !== 'jugador' && categoriasDisponibles.length > 0 && (
              <select value={categoriaActiva} onChange={handleCambioCategoria} style={{ padding: '8px 10px', background: '#111', border: '1px solid var(--border)', color: '#fff', borderRadius: '6px', outline: 'none', fontWeight: 800, cursor: 'pointer', fontSize: '0.85rem', width: '100%' }}>
                <option value="Todas">👉 TODAS LAS CATEGORÍAS</option>
                {categoriasDisponibles.map(c => <option key={c} value={c}>{c.toUpperCase()}</option>)}
              </select>
            )}

            <button onClick={() => setModoEdicion(!modoEdicion)} style={{ background: modoEdicion ? 'var(--accent)' : '#222', color: modoEdicion ? '#000' : '#fff', border: 'none', padding: '8px 12px', borderRadius: '6px', cursor: 'pointer', fontSize: '0.85rem', fontWeight: 'bold', width: esMovil ? '100%' : 'auto' }}>
              {modoEdicion ? '✅ Listo' : '⚙️ Personalizar'}
            </button>
            
            {esSuperUser && (
              <select value={clubActivo} onChange={handleCambioClub} style={{ padding: '8px 10px', background: '#111', border: '1px solid #c084fc', color: '#c084fc', borderRadius: '6px', outline: 'none', fontWeight: 800, cursor: 'pointer', fontSize: '0.85rem', width: '100%' }}>
        ...
        <>
          ...
          {modoEdicion && (
            <div style={{ background: '#111', padding: '15px', borderRadius: '8px', border: '1px dashed #444', marginBottom: '20px', animation: 'fadeIn 0.2s' }}>
              <h3 style={{ margin: '0 0 15px 0', fontSize: '0.9rem', color: '#fff' }}>¿Qué querés ver en tu inicio?</h3>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {widgetsPermitidos.map(w => {
                  const activo = misWidgetsActivos.includes(w.id);
                  return (
                    <button key={w.id} onClick={() => toggleWidget(w.id)} style={{ display: 'flex', alignItems: 'center', gap: '6px', background: activo ? 'rgba(255,255,255,0.1)' : 'transparent', border: `1px solid ${activo ? w.color || 'var(--accent)' : '#333'}`, color: activo ? '#fff' : '#666', padding: '6px 10px', borderRadius: '20px', cursor: 'pointer', transition: '0.2s', fontSize: '0.8rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: esMovil ? '1fr' : 'repeat(auto-fill, minmax(280px, 1fr))', gap: '15px', gridAutoFlow: 'dense' }}>
            ...
            {misWidgetsActivos.map(widgetId => {
              ...
              if (config.id === 'w_vep_anual') {
                ...
                return (
                  <div key={config.id} className="bento-card" style={{ gridColumn: esMovil ? '1 / -1' : 'span 2', minWidth: esMovil ? 'auto' : '300px', borderTop: '2px solid #3b82f6', padding: esMovil ? '15px' : '20px' }}>
                    <div className="stat-label" style={{ marginBottom: '15px', color: '#3b82f6' }}>{config.titulo} {new Date().getFullYear()} ({categoriaActiva})</div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: esMovil ? '6px' : '10px', textAlign: 'center' }}>
                      <div style={{ background: 'rgba(16, 185, 129, 0.1)', border: '1px solid rgba(16, 185, 129, 0.3)', padding: esMovil ? '10px' : '15px', borderRadius: '8px' }}>
                        <div style={{ color: '#10b981', fontSize: '1.8rem', fontWeight: 900 }}>{vepAnual.v}</div>
                        <div style={{ color: '#aaa', fontSize: '0.65rem', fontWeight: 'bold' }}>VICTORIAS</div>
                      </div>
                      <div style={{ background: 'rgba(245, 158, 11, 0.1)', border: '1px solid rgba(245, 158, 11, 0.3)', padding: esMovil ? '10px' : '15px', borderRadius: '8px' }}>
                        <div style={{ color: '#f59e0b', fontSize: '1.8rem', fontWeight: 900 }}>{vepAnual.e}</div>
                        <div style={{ color: '#aaa', fontSize: '0.65rem', fontWeight: 'bold' }}>EMPATES</div>
                      </div>
                      <div style={{ background: 'rgba(239, 68, 68, 0.1)', border: '1px solid rgba(239, 68, 68, 0.3)', padding: esMovil ? '10px' : '15px', borderRadius: '8px' }}>
                        <div style={{ color: '#ef4444', fontSize: '1.8rem', fontWeight: 900 }}>{vepAnual.d}</div>
                        <div style={{ color: '#aaa', fontSize: '0.65rem', fontWeight: 'bold' }}>DERROTAS</div>
                      </div>
                    </div>
              ...
              if (config.id === 'w_resultados_cat') {
                ...
                return (
                  <div key={config.id} className="bento-card" style={{ borderTop: '2px solid #8b5cf6', padding: esMovil ? '15px' : '20px' }}>
                      ...
                      {cats.length > 0 ? cats.map(cat => (
                        <div key={cat} style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                          ...
                          {resultadosRecientesCat[cat].map((d, idx) => {
                            ...
                            return (
                              <div key={`${cat}-${idx}`} onClick={() => navigate(`/resumen/${d.id}`)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#111', padding: '10px 12px', borderRadius: '6px', border: '1px solid #333', cursor: 'pointer', transition: 'border-color 0.2s' }} onMouseOver={(e) => e.currentTarget.style.borderColor = colorRes} onMouseOut={(e) => e.currentTarget.style.borderColor = '#333'}>
                                <div style={{ minWidth: 0, paddingRight: '10px' }}>
                                  <div style={{ fontSize: '0.65rem', color: 'var(--text-dim)', fontWeight: 800 }}>{d.fecha}</div>
                                  <div style={{ fontSize: '0.8rem', color: '#fff', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>vs {d.rival?.toUpperCase()}</div>
                                </div>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
                                  <strong style={{ fontSize: '1.1rem', color: '#fff' }}>{d.gf} - {d.gc}</strong>
                                  <span style={{ background: colorRes, color: '#000', width: '22px', height: '22px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '4px', fontWeight: 900, fontSize: '0.75rem' }}>{d.res}</span>
                                </div>
                              </div>
                            )
                          })}
              ...
              if (config.id === 'w_goles_cat') {
                ...
                return (
                  <div key={config.id} className="bento-card" style={{ borderTop: '2px solid #0ea5e9', padding: esMovil ? '15px' : '20px' }}>
                    <div className="stat-label" style={{ marginBottom: '15px', color: '#0ea5e9' }}>{config.titulo} ({new Date().getFullYear()})</div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      {cats.length > 0 ? cats.map(cat => (
                         <div key={cat} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#111', padding: '10px 12px', borderRadius: '6px', border: '1px solid #333' }}>
                           <span style={{ fontSize: '0.8rem', color: '#ccc', fontWeight: 'bold', textTransform: 'uppercase' }}>{cat}</span>
              ...
              if (config.id === 'w_proximo') {
                return (
                  <div key={config.id} className="bento-card" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', borderTop: '2px solid #10b981', padding: esMovil ? '15px' : '20px' }}>
                    <div className="stat-label" style={{ marginBottom: '15px', color: '#10b981' }}>{config.titulo}</div>
                    {proximoPartido ? (
                      <div style={{ background: '#111', padding: '15px', borderRadius: '6px', border: '1px solid #333' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                          <span style={{ fontSize: '0.7rem', color: '#10b981', fontWeight: 'bold' }}>📅 {proximoPartido.fecha?.split('-').reverse().join('/')}</span>
                        ...
                        <div style={{ fontSize: '1.1rem', fontWeight: 900, textAlign: 'center', margin: '15px 0', color: '#fff', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>vs {proximoPartido.rival?.toUpperCase()}</div>
                        <div style={{ textAlign: 'center', fontSize: '0.7rem', color: 'var(--text-dim)', fontWeight: 800 }}>📍 CONDICIÓN: {proximoPartido.condicion?.toUpperCase()}</div>
                      </div>
                    ) : (
                      <div style={{ textAlign: 'center', color: 'var(--text-dim)', padding: '20px', background: '#111', borderRadius: '6px', border: '1px dashed #333', fontSize: '0.85rem' }}>No hay partidos programados.</div>
                    )}
                  </div>
                );
              }
              ...
              if (config.id === 'w_ultimo') {
                return (
                  <div key={config.id} className="bento-card" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', borderTop: '2px solid var(--text-dim)', padding: esMovil ? '15px' : '20px' }}>
                    <div className="stat-label" style={{ marginBottom: '15px', color: 'var(--text-dim)' }}>{config.titulo}</div>
                    {ultimoPartido ? (
                      <div style={{ background: '#111', padding: '15px', borderRadius: '6px', border: '1px solid #333' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                          <span style={{ fontSize: '0.7rem', color: 'var(--text-dim)' }}>{ultimoPartido.fecha?.split('-').reverse().join('/')}</span>
                        ...
                        <div style={{ fontSize: '1.1rem', fontWeight: 900, textAlign: 'center', margin: '15px 0', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>vs {ultimoPartido.rival?.toUpperCase()}</div>
                        <button onClick={() => navigate(`/resumen/${ultimoPartido.id}`)} className="btn-secondary" style={{ width: '100%', fontSize: '0.75rem', padding: '10px' }}>VER REPORTE</button>
                      </div>
                    ) : (
                      <div style={{ textAlign: 'center', color: 'var(--text-dim)', padding: '20px', background: '#111', borderRadius: '6px', border: '1px dashed #333', fontSize: '0.85rem' }}>No hay partidos registrados aún.</div>
                    )}
                  </div>
                );
              }
              ...
              if (config.id === 'w_stats_base') {
                return (
                  <div key={config.id} className="bento-card" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', alignItems: 'center', padding: esMovil ? '15px' : '20px' }}>
                    <div style={{ textAlign: 'center', padding: '15px 10px', background: '#111', borderRadius: '6px', border: '1px solid #333' }}>
                      <div style={{ fontSize: '1.8rem', fontWeight: 900, color: '#fff' }}>{estadisticas.plantel}</div>
                      <div className="stat-label" style={{ fontSize: '0.6rem', marginTop: '5px' }}>JUGADORES ({categoriaActiva})</div>
                    </div>
                    <div style={{ textAlign: 'center', padding: '15px 10px', background: '#111', borderRadius: '6px', border: '1px solid #333' }}>
                      <div style={{ fontSize: '1.8rem', fontWeight: 900, color: 'var(--accent)' }}>{estadisticas.jugados}</div>
                      <div className="stat-label" style={{ fontSize: '0.6rem', marginTop: '5px' }}>PARTIDOS ({categoriaActiva})</div>
                    </div>
                  </div>
                );
              }
              ...
              if (config.tipo === 'link') {
                ...
                return (
                  <div key={config.id} className="bento-card" style={{ textAlign: 'center', padding: esMovil ? '20px 15px' : '30px 20px', cursor: 'pointer', border: `1px solid ${config.color || '#333'}`, background: `linear-gradient(180deg, ${bgSoft} 0%, rgba(0,0,0,0) 100%)`, transition: 'transform 0.2s', display: 'flex', flexDirection: 'column', justifyContent: 'center' }} onClick={() => navigate(config.ruta)}
                  onMouseOver={(e) => !esMovil && (e.currentTarget.style.transform = 'translateY(-5px)')}
                  onMouseOut={(e) => !esMovil && (e.currentTarget.style.transform = 'translateY(0)')}>
                    <div style={{ fontSize: esMovil ? '2rem' : '2.5rem', marginBottom: '8px' }}>{config.icon}</div>
                    <div className="stat-label" style={{ color: config.color || '#fff', fontSize: '0.9rem' }}>{config.titulo}</div>
                    <p style={{ color: 'var(--text-dim)', marginTop: '6px', fontSize: '0.75rem', lineHeight: '1.3', marginBottom: 0 }}>{config.desc}</p>
                  </div>
                );
              }
              return null;
