/* EL CÁLCULO DEL PLANTEL, FUERA DE LA PANTALLA
 *
 * Esto vivía dentro de un useMemo de Resumenplantel: 222 líneas encerradas en
 * una pantalla, así que para comparar dos jugadores no quedaba otra que
 * recalcular todo de nuevo o abrir dos pestañas.
 *
 * Acá es una función pura: entran los datos, salen las filas. Eso permite tres
 * cosas que antes no se podían: usarla desde otra pantalla, probarla sin
 * navegador, y más adelante contrastarla contra una versión en SQL sin tener
 * que adivinar si dan lo mismo.
 *
 * NO SE TOCÓ UNA SOLA CUENTA. Es el mismo código, movido. Cualquier diferencia
 * en los números sería un error de la mudanza, no una mejora, y para eso están
 * las pruebas de plantel.test.js.
 */
...
/**
 * Procesa el plantel entero y devuelve una fila por jugador de campo y una por
 * arquero, ya con minutos, rating, goles, duelos y todo lo demás.
 *
 * Las nueve entradas son exactamente las que tenía el useMemo original.
 */
export function procesarPlantel({
  raw,
  partidosScopeCat,
  filtroTorneo,
  filtroCategoria,
